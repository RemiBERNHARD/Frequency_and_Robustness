...
def fft_low_pass(input_tensor, lim_freq, nb_channels=3):
    ...
    mask = np.zeros((input_tensor.shape[2],input_tensor.shape[2]))
    crow = int(input_tensor.shape[2] /2)
    ccol = int(input_tensor.shape[2] /2)
    ...
    res_c1 = tf.math.real(inverse_fft) + tf.math.imag(inverse_fft)    
    ...
    inverse_fft = ifftshift(masked)
    inverse_fft = ifft2d(inverse_fft)
    res_c2 = tf.math.real(inverse_fft) + tf.math.imag(inverse_fft)
    ...
    inverse_fft = ifftshift(masked)
    inverse_fft = ifft2d(inverse_fft)
    res_c3 = tf.math.real(inverse_fft) + tf.math.imag(inverse_fft)        
...
def fft_high_pass(input_tensor, lim_freq, nb_channels=3):
    input_tensor_c = tf.cast(input_tensor, dtype=tf.complex64)
    mask = np.ones((input_tensor.shape[2],input_tensor.shape[2]))
    crow = int(input_tensor.shape[2] /2)
    ccol = int(input_tensor.shape[2] /2)
