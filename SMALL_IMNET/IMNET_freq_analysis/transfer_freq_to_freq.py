...
from utils_func import metrics, get_indices, imnet_load_data
...
model_for_weights_s = load_model("models/IMNET_" + model_source + ".h5")
model_for_weights_t = load_model("models/IMNET_" + target + ".h5")
...
type_freq_t = sys.argv[6]
print("Type of filtering for target model: " + type_freq_t)
lim_freq_t1 = int(sys.argv[7])
print("Limite freq for target model: " + str(lim_freq_t1))


model_input = Input((224,224,3))
model = MobileNetV2((224,224,3), alpha=1, nb_classes=10,
                        input_tensor=model_input, type_freq=type_freq_s, low_freq=lim_freq_s1)
for j in range(2, len(model.layers)):        
                model.layers[j].set_weights(model_for_weights_s.layers[j-1].get_weights())
model.compile(loss='categorical_crossentropy', optimizer="sgd", metrics=['accuracy'])

model_input = Input((224,224,3))
model_target = MobileNetV2((224,224,3), alpha=1, nb_classes=10,
                        input_tensor=model_input, type_freq=type_freq_t, low_freq=lim_freq_t1)
for j in range(2, len(model_target.layers)):        
                model_target.layers[j].set_weights(model_for_weights_t.layers[j-1].get_weights())
model_target.compile(loss='categorical_crossentropy', optimizer="sgd", metrics=['accuracy'])

print("Source model: Accuracy on freq test set: " + str(model.evaluate(X_test, Y_test, verbose=0)[1]))
print("Target model: accuracy on freq test set: " + str(model_target.evaluate(X_test, Y_test, verbose=0)[1]))
...
for i in range(0, len(indices),100):
    X_adv[i:(i+100)] = fgsm_attack.generate_np(X_test[indices[i:(i+100)]], **fgsm_params)
print("metrics")
...
X_adv = np.zeros((len(indices),224,224,3))
for i in range(0, len(indices),100):
    X_adv[i:(i+100)] = mim_attack.generate_np(X_test[indices[i:(i+100)]], **mim_params)
print("metrics")
print(metrics(model, X_adv, X_test, y_test, indices))    
print(metrics(model_target, X_adv, X_test, y_test, indices))    
