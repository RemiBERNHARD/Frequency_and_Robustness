...
if (model_type == "madry"):
    ...
    for step in np.arange(0, 80000):  
        ...
        if (step % 1000 ==0):
            ...
            model.save_weights("weights/svhn_weights_best_" + model_type + "_" + str(step) + "_" + str(acc_val) +
...
if (model_type == "madry_cons_all"):
    ...
    for step in np.arange(0, 80000):  
        ...
        if (step <= 1000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff_low: 0.02, coeff_high: 0.02,
                     K.learning_phase(): 1 })
        ...
        if (step > 10000) & (step <= 20000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff_low: 0.2, coeff_high: 0.2,
                     K.learning_phase(): 1 })
        if (step > 20000) & (step <= 40000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff_low: 0.25, coeff_high: 0.25,
                     K.learning_phase(): 1 })
        if (step > 40000) & (step <= 70000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff_low: 0.3, coeff_high: 0.3,
                     K.learning_phase(): 1 })    
        if (step > 70000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff_low: 0.4, coeff_high: 0.4,
                     K.learning_phase(): 1 })
        if (step % 100 == 0):
            print("step: " + str(step))
        ...
        if (step % 1000 == 0):
            ...
            print("Accuracy on hold-out adv examples: " + str(acc_adv_eval))
            print("Accuracy on training batch of adv examples: " + str(model.evaluate(x_batch_adv, y_batch, verbose=0)[1]))       
            model.save_weights("weights/svhn_weights_best_" + model_type + "_" + str(lim_freq_low) + "_" + str(lim_freq_high) +
    ...
    for step in np.arange(0,80000):  
        ...
        if (step > 1000) & (step <= 10000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff: 0.1,
                     K.learning_phase(): 1 })
        if (step > 10000) & (step <= 20000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff: 0.2,
                     K.learning_phase(): 1 })
        if (step > 20000) & (step <= 40000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff: 0.25,
                     K.learning_phase(): 1 })
        if (step > 40000) & (step <= 70000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff: 0.3,
                     K.learning_phase(): 1 })    
        if (step > 70000):
            sess.run([opt_op, model.updates], feed_dict={y: y_batch, model.inputs[0]: x_batch, coeff: 0.4,
                     K.learning_phase(): 1 })
        if (step % 100 == 0):
            print("step: " + str(step))
        ...
        if (step % 1000 == 0):
            ...
            print("Accuracy on hold-out adv examples: " + str(acc_adv_eval))
            print("Accuracy on training batch of adv examples: " + str(model.evaluate(x_batch_adv, y_batch, verbose=0)[1]))       
            model.save_weights("weights/svhn_weights_best_" + model_type + "_" + str(lim_freq) +
                               "_" + str(step) + "_" + str(acc_val) + "_" + str(acc_adv_eval) + ".hdf5")     
...
if (model_type == "madry_high"):    
    ...
    model_logits_low = model(x_highf)._op.inputs[0]
    cons_loss = tf.reduce_mean(tf.norm(model_logits - model_logits_low, ord=2, axis=1))
