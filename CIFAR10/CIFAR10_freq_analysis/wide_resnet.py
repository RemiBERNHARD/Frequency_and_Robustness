...
from keras.layers import Input, Add,  Dropout, Flatten, Dense, Lambda, Conv2D, AveragePooling2D, BatchNormalization
...
def create_wide_residual_network(input_dim, nb_classes=10, N=4, k=10, dropout=0.0, verbose=1, 
                                 input_tensor="no", type_freq=None, low_freq=0, soft_act="no"):
    """
    ...
    if (input_tensor != "no") & (type_freq != None):
        ip = input_tensor
        if (type_freq == "low"):
            j = Lambda(fft_low_pass, output_shape=input_dim, arguments={'lim_freq': low_freq})(ip)
        if (type_freq == "high"):
            j = Lambda(fft_high_pass, output_shape=input_dim, arguments={'lim_freq': low_freq})(ip)
        x = initial_conv(j)
    ...
    elif (input_tensor != "no") & (soft_act == "yes"):
        x = Dense(nb_classes, kernel_regularizer=l2(weight_decay), activation='softmax')(x)
    else :
