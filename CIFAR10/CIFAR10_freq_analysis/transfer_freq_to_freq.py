...
from utils_func import metrics, get_indices
...
model_for_weights_s = load_model("models/CIFAR10_" + model_source + ".h5")
model_for_weights_t = load_model("models/CIFAR10_" + target + ".h5")
...
model = create_wide_residual_network((32,32,3), nb_classes=10, N=4, k=8, dropout=0.1, verbose=1,
            input_tensor=model_input, type_freq=type_freq_s, low_freq=lim_freq_s1, soft_act="yes")
for j in range(2, len(model.layers)):        
                model.layers[j].set_weights(model_for_weights_s.layers[j-1].get_weights())
model.compile(loss='categorical_crossentropy', optimizer="sgd", metrics=['accuracy'])

model_input = Input((32,32,3))
model_target = create_wide_residual_network((32,32,3), nb_classes=10, N=4, k=8, dropout=0.1, verbose=1,
                  input_tensor=model_input, type_freq=type_freq_t, low_freq=lim_freq_t1, soft_act="yes")   
